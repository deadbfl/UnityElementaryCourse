...
public class CameraRotate : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(1))
            targetVector = new Vector3(0,player.transform.eulerAngles.y,0);

        if(PlayerAiming.instance.isAiming)
            player.transform.eulerAngles += new Vector3(0, mouseX * rotationSpeed * Time.deltaTime, 0);
